    ...
    return (
        <div className="relative group">
            ...
            <div
                ...
                className={cn(
                    "w-full flex items-center gap-3 px-3 py-3 rounded-lg text-left transition-all cursor-pointer",
                    isActive
                        ? "conv-active"
            ...
            >
                ...
                <div className="relative shrink-0">
                    ...
                    {(other?.isBot || other?.isOnline) && (
                        <span className="absolute bottom-0 right-0 size-2.5 rounded-full bg-green-500 ring-2 ring-sidebar online-pulse" />
                    )}
                </div>
                ...
                <div className="flex-1 min-w-0">
                    ...
                    {/* Status text (Feature 3) */}
                    {!isTyping && (other?.statusText || other?.statusEmoji) && (
                        <p className="truncate text-[10px] text-primary/70 italic leading-tight mb-0.5">
                            {other.statusEmoji} {other.statusText}
                        </p>
                    )}
                    <div className="flex items-center justify-between gap-1 mt-0.5">
                        <p className={cn(
                            "truncate text-xs",
                            isTyping ? "italic text-primary" : unread > 0 ? "text-foreground font-medium" : "text-muted-foreground"
                        )}>
                            {preview}
                        </p>
                        {unread > 0 && !isTyping && (
                            <span className="shrink-0 flex nexus-gradient-bg items-center justify-center min-w-5 h-5 rounded-full text-white text-[10px] font-bold px-1">
                                {unread > 99 ? "99+" : unread}
                            </span>
                        )}
                    </div>
...
export default function ConversationsList() {
    ...
    // Live-update custom status in sidebar (Feature 3)
    useEffect(() => {
        const onStatus = (data: { userId: string; statusText: string; statusEmoji: string }) => {
            setConversationsList((prev) =>
                prev.map((conv) => ({
                    ...conv,
                    members: conv.members.map((m) =>
                        m._id === data.userId
                            ? { ...m, statusText: data.statusText, statusEmoji: data.statusEmoji }
                            : m
                    ),
                }))
            )
        }
        socket.on("user-status-updated", onStatus)
        return () => { socket.off("user-status-updated", onStatus) }
                            ...
                            <p className="text-sm">
                                {query
                                    ...
                                    : filter === "unread"
                                        ? "No unread tickets"
                                        : filter === "online"
                                            ? "No active clients online right now"
