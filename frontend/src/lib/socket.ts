...
/** Feature: Message Reactions — toggle a reaction on a message */
export const emitReactMessage = (payload: { messageId: string; conversationId: string; emoji: string }): void => {
    socket.emit("react-message", payload);
};

/** Feature: Custom Status — set/update own status text & emoji */
export const emitSetStatus = (payload: { statusText: string; statusEmoji: string }): void => {
    socket.emit("set-status", payload);
};
