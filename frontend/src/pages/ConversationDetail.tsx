...
import ThreadDrawer from "@/components/dashboard/ThreadDrawer"
...
function StreamingBotBubble({ text }: { text: string }) {
    return (
        <div className="flex items-end gap-2 max-w-[85%] mr-auto">
            <div className="bubble-bot px-3.5 py-2 text-sm shadow-sm">
                ...
                <span className="inline-block w-1.5 h-3.5 ml-0.5 bg-primary/60 align-middle animate-pulse rounded-sm" />
            </div>
        </div>
    )
}
...
export default function ConversationDetail() {
    ...
    // ── socket: message-reaction-updated (Feature 1) ──────────────────────
    useEffect(() => {
        ...
        return () => { socket.off("message-reaction-updated", onReaction) }
    }, [id, setMessageList])

    // ── socket: user-status-updated (Feature 3) ───────────────────────────
    useEffect(() => {
        const onStatus = (data: { userId: string; statusText: string; statusEmoji: string }) => {
            setReceiver((prev) =>
                prev?._id === data.userId
                    ? { ...prev, statusText: data.statusText, statusEmoji: data.statusEmoji }
                    : prev
            )
        }
