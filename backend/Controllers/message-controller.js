...
const ai = new GoogleGenAI({ apiKey: GEMINI_API_KEY || "dummy-key-for-fallback" });
...
const streamAiResponse = async function* (text, senderId, conversationId) {
  ...
  let fullText = "";
  const isKeyPlaceholder = !GEMINI_API_KEY || GEMINI_API_KEY.includes("YOUR_GEMINI_API_KEY");

  // Only build Gemini chat history + object when we have a real API key
  let chat = null;
  if (!isKeyPlaceholder) {
  ...
  if (isKeyPlaceholder) {
    // Elegant simulated support streaming fallback!
    console.log("Simulating AI response stream fallback (No API key)...");
    const replies = [
      "Hello! I am Nexus, your B2B AI Support Assistant. I've analyzed your request regarding the support ticketing pipeline. It looks like you're experiencing integration latencies. Let me verify the OAuth redirect callback setup for you.",
      "Greetings from the Nexus GenAI Copilot! I have flagged this support session as highly important. To assist you with your queries, here is our quick-start routing documentation: https://docs.nexus.com. What specific feature can I guide you with?",
      "Thank you for reaching out! Our GenAI SLA analysis detects a minor authentication challenge in your backend connection. Let's make sure your MongoDB replica sets and JWT secret keys are fully updated. Let me know if you would like a step-by-step debug protocol!"
    ];
    const replyText = text.toLowerCase().includes("auth") || text.toLowerCase().includes("login")
      ? "Nexus AI Assistant here! I see you are inquiring about customer authentication. Please verify that your JWT token signature length matches 256 bits, and your client cookies are configured as HTTPOnly."
      : replies[Math.floor(Math.random() * replies.length)];

    // Stream the response back in 5-character chunks with a small sleep delay
    const chunks = [];
    for (let i = 0; i < replyText.length; i += 8) {
      chunks.push(replyText.slice(i, i + 8));
    }
    ...
    try {
      ...
      console.log("Falling back to AI simulation upon API error...");
      const fallbackText = "Nexus AI Assistant here! I detected a slight API connection delay, but I am still online to help you. We are analyzing your active support session parameters to optimize resolution speed.";
      const chunks = [];
...
/**
 * GET /api/message/:id/thread
 * Returns all messages that are replies to this message (thread view).
 */
