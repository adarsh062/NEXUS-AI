...
module.exports = (io, socket, userSocketMap) => {
  ...
  // ─── React to message ──────────────────────────────────────────────────────
  // emoji: the emoji string, add: true to add, false to remove
  socket.on("react-message", async (data) => {
    try {
      ...
      if (!message || message.softDeleted) return;

      // Toggle: if user already reacted with same emoji → remove it, otherwise add it
  ...
  socket.on("set-status", async (data) => {
    try {
      const { statusText, statusEmoji } = data;
      const userId = currentUserId;

      await User.findByIdAndUpdate(userId, {
        statusText: (statusText || "").slice(0, 60),
      ...
      conversations.forEach((c) => {
        c.members.forEach((m) => {
          if (m.toString() !== userId) friendIds.add(m.toString());
        });
      });

      friendIds.forEach((friendId) => {
        io.to(friendId).emit("user-status-updated", {
          userId,
          statusText: (statusText || "").slice(0, 60),
          statusEmoji: statusEmoji || "",
        });
      ...
      console.error("Error in set-status handler:", error);
    }
  });
